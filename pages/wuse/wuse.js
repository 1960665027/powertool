...
Page({
  ...
  radioChange3: function (e) {
    console.log('radio发生change事件，携带value值为：', e.detail.value)
    app.globalData.wuse = e.detail.value;
    
    app.globalData.jia = parseFloat(app.globalData.item1 + app.globalData.item2 + app.globalData.wuse)
    this.setData({
      jieguo: app.globalData.jia
    })
    //绘图
    ctx.setFillStyle(this.data.items3[app.globalData.wuse].color)
    ctx.fillRect(166, 5, 6, 22)
    ctx.draw(true)
  },
  radioChange4: function (e) {
    app.globalData.item4 = parseFloat(e.detail.value);
    console.log('radio发生change事件，携带value值为：', e.detail.value)
    this.setData({
      jieguo: app.globalData.item4
    })
    switch (app.globalData.item4) {
      case 1:
        //绘图
  ...
  radioChange5: function (e) {
   let wucha = e.detail.value;
    console.log('radio发生change事件，携带value值为：', e.detail.value)
    let jieguo = app.globalData.jia * app.globalData.item4
    this.setData({
      jieguo: jieguo,
      wucha: wucha
    })
